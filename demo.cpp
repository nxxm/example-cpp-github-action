#include <iostream>

int main(int argc, const char* argv[]) {

  std::cout << "😉 Welcome into a relaxing C++ experience !" << std::endl; //English
  std::cout << "😉 Bienvenue dans une expérience C++ relaxante !" << std::endl; //French
  std::cout << "😉 Willkommen zu einer entspannenden C++ Erfahrung !" << std::endl; //German
  std::cout << "😉 Bienvenido a una relajante experiencia en C++ !" << std::endl; //Spanish
  std::cout << "😉 Bem-vindo a uma experiência relaxante em C++ !" << std::endl; //Portuguese
  std::cout << "😉 Benvenuti in una rilassante esperienza C++ !" << std::endl; //Italian
  return 0;
}
